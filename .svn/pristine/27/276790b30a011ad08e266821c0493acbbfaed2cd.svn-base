...
public class TinyIPFIXParser extends IPFIXParser {
	...
	public byte[] translatePacket(byte[] packet, InetAddress source) throws ParseException {
		try {
			...
			if (_isTinyIPFIX(packet)) {
				packet = _translateHeader(packet, source);
			}
	 ...
	 */
	@Override
	protected int _getSupportedVersion() {
		return 0xA;
	}
	 ...
	 * @param packet
	 */
	protected boolean _isTinyIPFIX(byte[] packet) {
		try {
			...
			if (version != _getSupportedVersion() || packet.length == 10) {
				return true;
			}
	 ...
	 * @param packet
	 */
	protected byte[] _translateHeader(byte[] packet, InetAddress source) throws ParseException {
